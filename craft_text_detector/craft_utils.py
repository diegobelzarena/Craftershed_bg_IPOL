...
def adjustResultCoordinates(polys, ratio_w, ratio_h, ratio_net=2):
    if len(polys) > 0:
        ...
        valid_polys = [p for p in polys if p is not None and isinstance(p, np.ndarray)]
        for k in range(len(valid_polys)):
            valid_polys[k] = valid_polys[k] * ratio_net
            valid_polys[k][:, 0] *= ratio_w
            valid_polys[k][:, 1] *= ratio_h
        return valid_polys
    return polys
